...
public class Main {
    ...
    private static void viewAllTasks() {
        ...
        if (tasks.isEmpty()) {
            ...
            for (Task task : tasks) {
                System.out.printf("Задача %s: Id - %s, статус - %s. Описание - %s.\n", task.getTitle(), task.getId(), task.getStatus(), task.getDescription());
            }
        }
    }
    ...
    private static void viewAllEpics() {
        ...
        if (epics.isEmpty()) {
            ...
            for (Epic epic : epics) {
                System.out.printf("Эпик %s: Id - %s, статус - %s. Описание - %s.\n", epic.getTitle(), epic.getId(), epic.getStatus(), epic.getDescription());
            }
        }
    }
    ...
    private static void viewSubtasksOfEpic() {
        ...
        if (subtasks.isEmpty()) {
            ...
            for (Subtask subtask : subtasks) {
                System.out.printf("Подзадача %s: Id - %s, статус - %s. Описание - %s.\n", subtask.getTitle(), subtask.getId(), subtask.getStatus(), subtask.getDescription());
            }
        }
    }
    private static void createSubtask() {
        ...
        String description = scanner.next();
        manager.createSubtask(new Subtask(name, description, epic));
        ...
        if (history.isEmpty()) {
            ...
            for (Task task : history) {
                if (task instanceof Epic){
                    ...
                    System.out.printf("Подзадача %s: Id - %s, статус - %s. Описание - %s.\n", task.getTitle(), task.getId(), task.getStatus(), task.getDescription());
                } else System.out.printf("Задача %s: Id - %s, статус - %s. Описание - %s.\n", task.getTitle(), task.getId(), task.getStatus(), task.getDescription());
            }
        }
    }
}